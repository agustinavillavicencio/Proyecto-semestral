...
document.addEventListener("DOMContentLoaded", function () {
  // Información que aparecerá en el popup
  const infoQueens = {
  ...
  document.querySelectorAll('.artista-img').forEach(img => {
    img.addEventListener('click', () => {
      ...
      const contenidoPopup = document.getElementById('contenido-popup-queen');

      contenidoPopup.textContent = contenido;
      popup.style.display = 'block';
    });
  });

  // Cerrar el popup
  const botonCerrar = document.querySelector('.cerrar-queen');
  if (botonCerrar) {
